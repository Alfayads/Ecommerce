    ...
    <div>
      ...
      {addresses && addresses.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {addresses.map((addr) => (
            <AddressCard key={addr.id} address={addr} isPrimary={addr.id === primaryAddressId} onEdit={() => handleOpenForm(addr)} onDelete={() => handleDelete(addr.id)} onSetPrimary={() => handleSetPrimary(addr.id)} />
          ))}
        </div>
      ) : (
